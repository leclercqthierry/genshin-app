...
import {
    localMaterialSchema,
    type LocalMaterialSchema,
} from "@/domain/local-material/schema";

import type { LocalMaterialFormState } from "./types";

const initialState: LocalMaterialFormState = {
    success: false,
    errors: {},
};
...
export default function LocalMaterialForm({ action, submitLabel, defaultValues }: Props) {
    ...
    } = useForm<LocalMaterialSchema>({
        resolver: zodResolver(localMaterialSchema),
        mode: "onChange",
        defaultValues: {
    ...
    useEffect(() => {
        if (state.success) {
            router.push("/admin/local-materials");
        }
    ...
    return (
        ...
        >
            <AppFormSection withBorder={false}>
                {/* Nom */}
                <AppFieldBase
                    label="Nom de la ressource locale"
                    name="name"
                    required
                    error={errors.name?.message || state.errors?.name?.[0]}
                >
                    <AppInput
                        placeholder="Lys calla"
                        {...register("name")}
                    />
                </AppFieldBase>
                        ...
                        {iconUrl && (
                            <div className="flex justify-center mb-3">
                                <Image
                                    ...
                                    width={80}
                                    height={80}
                                    className="rounded border object-contain bg-rarity-1"
                                />
                            </div>
                        )}
