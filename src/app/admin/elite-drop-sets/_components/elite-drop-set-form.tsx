...
import {
    eliteDropSetSchema,
    type EliteDropSetSchema,
} from "@/domain/elite-drop-set/schema";

import type { EliteDropSetFormState } from "./types";

const initialState: EliteDropSetFormState = {
    success: false,
    errors: {},
};
...
type Props = {
    action: (
        prev: EliteDropSetFormState,
        formData: FormData
    ) => Promise<EliteDropSetFormState>;
    submitLabel: string;
    defaultValues?: Partial<EliteDropSetSchema>;
};

export default function EliteDropSetForm({
    action,
    submitLabel,
    defaultValues,
}: Props) {
    ...
    } = useForm<EliteDropSetSchema>({
        resolver: zodResolver(eliteDropSetSchema),
        mode: "onChange",
        defaultValues: {
    ...
    useEffect(() => {
        if (state.success) {
            router.push("/admin/elite-drop-sets");
        }
        ...
        <AppFormWrapper action={formAction} size="md" className="mx-auto max-w-md lg:max-w-2xl">
            <AppSection variant="ghost">
                ...
                <div className="lg:max-w-1/2 max-w-md mx-auto">
                    ...
                    >
                        <AppInput placeholder="Ex: Corne" {...register("name")} />
                    </AppFieldBase>
                </div>
