...
import {
    aptitudeSetSchema,
    type AptitudeSetSchema,
} from "@/domain/aptitude-set/schema";

import type { AptitudeSetFormState } from "./types";

const initialState: AptitudeSetFormState = {
    success: false,
    errors: {},
};
...
type Props = {
    action: (
        prev: AptitudeSetFormState,
        formData: FormData
    ) => Promise<AptitudeSetFormState>;
    submitLabel: string;
    defaultValues?: Partial<AptitudeSetSchema>;
};

export default function AptitudeSetForm({
    action,
    submitLabel,
    defaultValues,
}: Props) {
    ...
    } = useForm<AptitudeSetSchema>({
        resolver: zodResolver(aptitudeSetSchema),
        mode: "onChange",
        defaultValues: {
    ...
    useEffect(() => {
        if (state.success) {
            router.push("/admin/aptitude-sets");
        }
